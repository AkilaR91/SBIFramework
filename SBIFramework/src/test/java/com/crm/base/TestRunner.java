...
public class TestRunner {
	...
	public static void main(String[] args) throws ClassNotFoundException {
		...
		if (args.length != 0) {
			runID = args[0];
			clientID = args[1];
			System.out.println("Run ID from command line arguments: RunID: "+ runID + " ClientID: "+ clientID);
	...
	public static void setConfig() {
		...
		config.setProperty("ClientID", clientID);
		config.setProperty("RunExecutedFromJar", "YES");
		config.setProperty("StatusFile", "\\AutomationsFiles\\log_"+runID+".txt");
		config.setProperty("LogoName", "..\\Automation_script_"+clientID+"\\resources\\logos\\crmimg.jpg");

		try {
			config.store(out, "Modified properties according to runID and clientID");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}finally {
